...
EXCLUDED_DIRS = {
    ...
    "alembic", "dist", "build", ".mypy_cache", ".ruff_cache",
    "tests", ".reasonix",  # tests have different rules
}
...
def main():
    ...
    # Summary
    l0_l1_failures = sum(
        1 for r in results if not r.passed and r.level in (Level.L0, Level.L1)
    )
    l2_warnings = sum(
        1 for r in results if not r.passed and r.level == Level.L2
    )
    total_violations = sum(len(r.findings) for r in results if not r.passed)

    print(file=sys.stderr)
    print(f"Checks: {len(results)} run | {l0_l1_failures} failed | {l2_warnings} warnings | {total_violations} violations",
          file=sys.stderr)
